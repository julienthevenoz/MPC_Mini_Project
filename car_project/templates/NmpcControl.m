classdef NmpcControl < handle
    ...
    methods
        function obj = NmpcControl(car, H)
            ...
            % opti.subject_to((0 <= obj.U(2,:))&(obj.U(2,:)  <= 1));  %limit throttle
            % opti.subject_to((0 <= obj.U(1,:)) &(obj.U(1,:) <= 0.5236)); %limit steering to 30 degrees 
            % opti.subject_to((-0.5 <= obj.X(2,:)) & (obj.X(2,:)  <= 3.5 )); % car mustn't go out of the road
            % opti.subject_to((0 <= obj.X(3,:)) & (obj.X(3,:) <= 0.0873));  %car angle theta must be smaller than 5 degrees
            
            opti.subject_to(0 <= obj.U(2,:));  %limit throttle
            opti.subject_to(obj.U(2,:)  <= 1);
            opti.subject_to(0 <= obj.U(1,:)); %limit steering to 30 degrees 
            opti.subject_to(obj.U(1,:) <= 0.5236);
            opti.subject_to(-0.5 <= obj.X(2,:)); % car mustn't go out of the road
            opti.subject_to(obj.X(2,:)  <= 3.5 );
            opti.subject_to(0 <= obj.X(3,:));  %car angle theta must be smaller than 5 degrees
