classdef NmpcControl < handle
    ...
    methods
        function obj = NmpcControl(car, H)
            ...
            h = car.Ts;   %h is the sample period of RK4. What should it be ?
            ...
            opti.minimize(...
              ...
              1*obj.U(1,:)*obj.U(1,:)' + ... % Minimize steering
              1*obj.U(2,:)*obj.U(2,:)' );% Minimize throttle
            ...
            opti.subject_to(-1 <= obj.U(2,:) <= 1);  %limit throttle
            opti.subject_to(-0.5236 <= obj.U(1,:) <= 0.5236); %limit steering to 30 degrees 
            opti.subject_to(-0.5 <= obj.X(2,:) <= 3.5 ); % car mustn't go out of the road
            opti.subject_to(-0.0873 <= obj.X(3,:) <= 0.0873);  %car angle theta must be smaller than 5 degrees
